...
def load_gs_mat_from_arrow(arrow_file_dir):
    ...
    for arrow_file_path in arrow_files:
        ...
        for ch in chr_keys:
            ...
            x = chr_mat_arrow['data']
            i = chr_mat_arrow['indices']
            idxptr = chr_mat_arrow['indptr']
            dim = (len(chr_mat_arrow['rowSums'][0]), len(chr_mat_arrow['colSums'][0]))
            chr_peak_mat = scipy.sparse.csc_matrix((x, i, idxptr), shape=dim).todense()
            
            if dim[1] != cell_num:
                # this piece of code was written for legacy ArchR before 1.0.2
                # this issues might not happen with the new version
...
def load_gs_mat_from_arrow_legacy(arrow_file_dir):
    """
    arrow_file_dir: directory containing .arrow files of all replicates/samples
    """
    if arrow_file_dir[-1] != '/':
...
def load_motif_mat_from_arrow(arrow_file_dir):
    ...
    for arrow_file_path in arrow_files:
        ...
        x = deviation_mat['data']
        i = deviation_mat['indices']
        idxptr = deviation_mat['indptr']
        n_rows = deviation_mat['rowMeans'].shape[1]
        n_cols = idxptr.shape[0]-1
        dim = (n_rows, n_cols)
        deviation_mat = scipy.sparse.csc_matrix((x, i, idxptr), shape=dim).todense()
        ...
        n_cols = idxptr.shape[0]-1
        dim = (n_rows, n_cols)
        z_mat = scipy.sparse.csc_matrix((x, i, idxptr), shape=dim).todense()
...
def load_motif_mat_from_arrow_legacy(arrow_file_dir):
    """
    arrow_file_dir: directory containing .arrow files of all replicates/samples
    ArchR changed sparse matrix format in 1.0.2. Use this to load Arrow files generated
